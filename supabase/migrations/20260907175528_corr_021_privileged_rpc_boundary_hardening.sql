...
alter function private.resolve_current_global_authority()
owner to postgres;

revoke execute on function private.resolve_current_global_authority()
from public, anon, authenticated, service_role, supabase_auth_admin;

grant usage on schema private
to authenticated;

grant execute on function private.resolve_current_global_authority()
to authenticated;
...
as $function$
  select
    internal_authority.identity_resolved,
    internal_authority.is_super_admin,
    internal_authority.has_company_membership
  from private.resolve_current_global_authority() as internal_authority;
$function$;

alter function public.resolve_current_global_authority()
owner to postgres;
